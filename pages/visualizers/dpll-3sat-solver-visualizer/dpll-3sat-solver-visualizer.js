...
class DPLLVisualizer {
    ...
    loadSample3SAT() {
        ...
        this.treeRoot.x = this.treeRoot.targetX = (this.canvas.width / (window.devicePixelRatio || 1)) / 2;
    ...
    *dpll() {
        ...
        for (let clause of this.baseClauses) {
            ...
            if (clauseStatus === 'UNDETERMINED') activeClausesCount++;
        }
        ...
        if (conflictDetected) {
            ...
            yield { msg: 'Conflict detected (Empty Clause generated)! Backtracking call stack sequence...' };
            return false;
        }
        ...
        if (unitLit) {
            ...
            yield { msg: `Unit Propagation: Forcing ${unitLit.variable} = ${targetVal.toString().toUpperCase()}` };
        ...
        if (pureLit) {
            ...
            yield { msg: `Pure Literal Rule: Eliminating via ${pureLit.variable} = ${targetVal.toString().toUpperCase()}` };
    ...
    calculateTreeCoordinates(node, xMin, xMax, y, levelHeight) {
        ...
        node.children.forEach((child, idx) => {
            this.calculateTreeCoordinates(child, xMin + (idx * sector), xMin + ((idx + 1) * sector), y + levelHeight, levelHeight);
        });
    }
    ...
    paintConnections(node) {
        ...
        node.children.forEach(child => {
            ...
            if (child.isSuccess) {
                this.ctx.strokeStyle = '#10b981'; this.ctx.lineWidth = 3.5;
            } else if (child.isBacktracked) {
                this.ctx.strokeStyle = 'rgba(244,63,94,0.15)'; this.ctx.lineWidth = 1.5;
            } else {
    ...
    paintNodes(node) {
        ...
        if (node === this.activeNode && node.variable !== 'ROOT') {
            ...
            strokeColor = '#10b981'; fillColor = 'rgba(16,185,129,0.15)';
        } else if (node.isBacktracked) {
            strokeColor = '#f43f5e'; opacity: 0.3; fillColor = 'rgba(244,63,94,0.05)';
        } else {
            switch (node.branchType) {
                case 'BRANCH': strokeColor = '#7c3aed'; break;
                case 'UNIT': strokeColor = '#06b6d4'; break;
                case 'PURE': strokeColor = '#f59e0b'; break;
            }
        }
